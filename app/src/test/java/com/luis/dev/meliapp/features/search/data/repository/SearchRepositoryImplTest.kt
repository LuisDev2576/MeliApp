...
class SearchRepositoryImplTest {
    ...
    private val mockDataSource = mockk<SearchDataSource>()
    ...
    fun setup() {
        ...
        // Instanciar el repositorio con el dataSource mockeado
        repository = SearchRepositoryImpl(
            dataSource = mockDataSource,
            defaultSiteId = "MLA" // Puedes cambiarlo si es necesario
        )
    }
    ...
    @Test
    fun `searchItems calls dataSource with siteId and query and returns response`() = runTest {
        // Dado que el dataSource retorne un SearchResponse con resultados
        val query = "macbook"
        ...
        // Cuando llamamos a searchItems en el repository
        val result = repository.searchItems(query)

        // Verificamos que se llame al dataSource con los valores correctos
        coVerify(exactly = 1) { mockDataSource.searchItems("MLA", query) }

        // Y el resultado sea el mismo que el fakeResponse
