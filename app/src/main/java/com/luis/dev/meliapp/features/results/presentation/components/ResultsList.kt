 ...
 *
 * @param results La lista de objetos SearchResultItem a mostrar.
 */
@Composable
fun ResultsList(results: List<SearchResultItem>, onProductClick: (String) -> Unit) {
    LazyColumn(
        modifier = Modifier
        ...
        items(results) { item ->
            ResultItem(item = item) {
                ...
                Log.d("ItemPrice", "ItemId ${item.id} / ItemPrice ${item.price} / ItemOriginalPrice ${item.originalPrice}")
            }
        }
    }
}
...
fun ResultsListPreview() {
    val item1 = SearchResultItem(
        ...
        permalink = "https://www.mercadolibre.com.ar/apple-macbook-air-13-pulgadas-2020-chip-m1-256-gb-de-ssd-8-gb-de-ram-oro/" +
            "p/MLA17828522#wid=MLA1449414435&sid=unknown",
